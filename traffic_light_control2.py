...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        model = Model(inputs=[input_1, input_2, input_3, input_4], outputs=[x])
    ...
    def act(self, state):
        if np.random.rand() <= self.epsilon: #explore vs exploit
...
if __name__ == '__main__':
    ...
    agent = DQNAgent()
    try:
        agent.load('NewModels4/reinf_traf_control9.h5')
    except:
    ...
    for e in range(episodes):
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and stepz < 7000:
            ...
            if(action == 0 and light[0][0][0] == 0):
                ...
                reward1 = traci.edge.getLastStepVehicleNumber(
                    '1si') + traci.edge.getLastStepVehicleNumber('2si') #all vehicles stopped in horizontal lane
                reward2 = traci.edge.getLastStepHaltingNumber(
                    '3si') + traci.edge.getLastStepHaltingNumber('4si') #all vehicles moving in vertical
                for i in range(10):
                    stepz += 1
        ...
        log.write(str(e) + ' ' + str(waiting_time)+'\n')
        log.close()
        print('episode - ' + str(e) + ' total waiting time - ' + str(waiting_time))
        agent.save('NewModels4/reinf_traf_control2_' + str(e) + '.h5')
